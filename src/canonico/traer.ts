...
export async function traerOfertas(eq: Equivalencia, descartes?: Descartes): Promise<Oferta[]> {
  ...
  if (necesitaFicha(cfg) && eq.url) {
    try {
      ...
      // pedir. Solo se le asigna cuando la pagina devolvio un unico producto:
      // una ficha incrusta ademas productos relacionados, y darles a todos la
      // url de la pagina haria que el calce por url eligiera uno de ellos.
      if (ofertas.length > 0) {
        return ofertas.length === 1 ? [{ ...ofertas[0]!, url: ofertas[0]!.url ?? eq.url }] : ofertas;
      }
    } catch (e) {
